...
The durable conversation index uses the same write/read-back retry mitigation for last-write-wins
races. Because `ctx.state` exposes no compare-and-set transaction, two worker processes can still race
