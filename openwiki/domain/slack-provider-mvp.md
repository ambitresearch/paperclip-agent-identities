...
| **Transport diagnostic misattributed as agent content** (DRO-1162: an ACPX/adapter transport warning, e.g. "Model metadata not found, defaulting to fallback metadata", streamed to Slack as if the agent authored it) | `SlackSessionReplyAccumulator` (`src/providers/slack/ingress/session-reply.ts`) accepts an `acpx.text_delta` as reply content **only** when it carries the DRO-1183 discriminator `provenance: "assistant"` on `channel: "output"`. Core classifies fail-closed (an untagged or unrecognized delta is `transport`, never `assistant`), so a transport warning arriving before the first genuine assistant delta can never become agent message content — see `docs/adapters/acpx-event-provenance.md` in core. Records with `provenance` of `tool`/`transport`/`error`/unknown, and assistant deltas on the `thought` channel, are dropped. Records with **no** `provenance` field are the pre-DRO-1183 ambiguous shape (`channel: "output"` + `tag: "agent_message_chunk"`), where a diagnostic and genuine prose are structurally indistinguishable; the DRO-1162 fail-closed guard is retained for that shape during the bounded transition window and can be removed once no live surface replays pre-contract run logs. `paperclipai/paperclip#1465` covers only the terminal surface, not this downstream classification. Filtering here is based on event type/channel/tag/provenance, never text-pattern/model-name matching. Confirmed final records and non-JSON adapter fallback still preserve legitimate answers, including prose that discusses or quotes a warning. Lifecycle JSONL, `acpx.status`/`acpx.error`, tool-call records, reasoning/thought-channel deltas, and raw stderr never reach Slack reply content. |
