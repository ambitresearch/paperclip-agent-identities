...
| **Transport diagnostic misattributed as agent content** (DRO-1162: an ACPX/adapter transport warning, e.g. "Model metadata not found, defaulting to fallback metadata", streamed to Slack as if the agent authored it) | `SlackSessionReplyAccumulator` (`src/providers/slack/ingress/session-reply.ts`) treats `acpx.text_delta` + `channel: "output"` + `tag: "agent_message_chunk"` as unconfirmed: it accumulates but never live-streams this source, and only delivers it (as a bounded fallback) if no higher-priority structured record (`result`, `item.completed`, Claude `content_block_delta`, Gemini/assistant message) ever supersedes it. This is a **plugin-side mitigation only** — the shape carries no provenance/kind discriminator today, so a diagnostic and genuine prose remain structurally indistinguishable; the primary fix is an upstream ACPX/core event-contract change (tracked at `paperclipai/paperclip#1465`, which currently covers only the terminal surface, not this downstream classification). Filtering here is intentionally based on event type/channel/tag plus priority ordering, never text-pattern/model-name matching, so legitimate answers that discuss or quote a warning are never suppressed. Lifecycle JSONL, `acpx.status`/`acpx.error`, tool-call records, reasoning/thought-channel deltas, and raw stderr already never reach `structured`/`streamableText` and are unaffected. |
