...
- Only after the clean release PR merges does an idempotent finalizer tag that exact merge commit, create a GitHub Release, and dispatch npm publication of the matching immutable tag. Comments or failed checks leave the release PR open and prevent tagging or publication; a rerun resumes the same release PR.
