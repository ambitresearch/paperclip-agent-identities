...
- `/tests/plugin.spec.ts`
  ...
  - sidecar writes/deletes and legacy settings fallback
- `/tests/identity-policy.spec.ts`
  - config parsing and missing-agent fail-closed behavior
  - default and explicit repository patterns
...
- `/tests/create-pull-request.spec.ts`
  ...
  - repo policy before secret resolution
...
- `/tests/repo-policy.spec.ts`
  - settings-level `validateRepoPolicy()` behavior
...
- Tests set `PAPERCLIP_AGENT_IDENTITIES_CREDENTIALS` to temporary files; production default is `/paperclip/.paperclip/agent-identities/credentials.json`, with a legacy `/paperclip/.paperclip/github-bot-identity/credentials.json` fallback when the renamed default is absent.
