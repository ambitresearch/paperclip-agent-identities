...
import { useSlackSettingsPanel, SlackSettingsPanel } from "../providers/slack/settings-adapter-ui.js";
import {
...
export function SettingsPage(props: PluginSettingsPageProps) {
  ...
  // Slack's setup-wizard state, effects, and handlers are owned by
  // src/providers/slack/settings-adapter-ui.tsx (DRO-1039) rather than
  // inlined here -- this hook mirrors the previous inline restore-on-mount
  // effect and handler bodies verbatim.
  const slackPanel = useSlackSettingsPanel({
    config: config && config.provider === SLACK_IDENTITY_PROVIDER_ID ? config : null,
    updateField: (field, value) => updateField(field as keyof IdentityFormState, value),
    setPreviousAgentId: (agentId) => setFormState((prev) => (prev ? { ...prev, previousAgentId: agentId } : prev)),
  ...
  const { slackSaveResult, slackSaveBusy } = slackPanel;
        ...
        <div style={dialogBackdropStyle} role="presentation">
          ...
          {activeFormSection === "slack" && (
            <SlackSettingsPanel
              ...
              hasSecretOptions={hasSecretOptions}
              formatSecretOption={formatSecretOption}
              getSecretFieldHint={getSecretFieldHint}
              companyId={companyId}
...
// copyTextToClipboard now lives in src/providers/slack/settings-adapter-ui.tsx
// alongside the Slack panel that uses it (DRO-1039).
...
// Slack manifest-flow state persistence (writeSlackManifestFlowState /
// readSlackManifestFlowState / deleteSlackManifestFlowState) now lives in
// src/providers/slack/settings-adapter-ui.tsx alongside the rest of the
// Slack settings-UI surface (DRO-1039).
