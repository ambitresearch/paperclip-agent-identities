...
import { githubListIssueCommentsToolSpec } from "../../../src/providers/github/tools/list-issue-comments.js";
...
describe("githubListIssueCommentsToolSpec.validateParams", () => {
  it("rejects a missing repository", () => {
    expect(
      githubListIssueCommentsToolSpec.validateParams({ issueNumber: 1 })
    ).toEqual({ ok: false, error: 'repository is required (e.g. "my-org/my-repo")' });
  });

  it("rejects a missing/invalid issueNumber", () => {
    expect(
      githubListIssueCommentsToolSpec.validateParams({ repository: "acme/widgets" })
    ).toEqual({ ok: false, error: "issueNumber must be a positive integer" });
  });

  it("rejects perPage over the cap", () => {
    expect(
      githubListIssueCommentsToolSpec.validateParams({ repository: "acme/widgets", issueNumber: 1, perPage: 500 })
    ).toEqual({ ok: false, error: "perPage must be a positive integer up to 100 if provided" });
  });

  it("accepts valid params", () => {
    const res = githubListIssueCommentsToolSpec.validateParams({
      repository: "acme/widgets", issueNumber: 1, page: 2, perPage: 50
    });
    expect(res.ok).toBe(true);
  });
});

describe("githubListIssueCommentsToolSpec.resolveResourceRef", () => {
  it("normalizes the repository into a github-repo ref", async () => {
    const res = await githubListIssueCommentsToolSpec.resolveResourceRef!({
...
describe("githubListIssueCommentsToolSpec.perform", () => {
  ...
  it("fails closed when the resolved token is null", async () => {
    const result = (await githubListIssueCommentsToolSpec.perform(execution(null))) as { error: string };
    expect(result.error).toBe("Internal error: missing resolved credential.");
  });

  it("lists comments and reports hasMore from the Link header", async () => {
    const fetchImpl = vi.fn(async (url: string) => {
      expect(url).toBe("https://api.github.com/repos/acme/widgets/issues/7/comments?page=1&per_page=30");
      return new Response(
        JSON.stringify([{ id: 1, user: { login: "alice" }, body: "hi", created_at: "2024-01-01T00:00:00Z", html_url: "https://github.com/x" }]),
    ...
    const result = (await githubListIssueCommentsToolSpec.perform(exec)) as { data: { comments: unknown[]; hasMore: boolean } };
  ...
  it("returns a generic error on network failure without leaking the token", async () => {
    ...
    (exec as { ctx: unknown }).ctx = buildCtx(fetchImpl as never);
    const result = (await githubListIssueCommentsToolSpec.perform(exec)) as { error: string };
  ...
  it("logs activity metadata without ever including the token", async () => {
    ...
    await githubListIssueCommentsToolSpec.perform(exec);
